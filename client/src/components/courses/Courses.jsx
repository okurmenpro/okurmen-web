...
const Courses = () => {
  ...
  const [activeTab, setActiveTab] = useState("nextStreams");

  const tabs = [
    { key: "nextStreams", label: t("nextStreams") },
    { key: "forChildren", label: t("forChildren") },
  ...
  const translatedSchedule = (schedule) => t('perWeek', { count: schedule });
      ...
      <div className="flex scrollbar-hide space-x-4 mb-8 overflow-x-auto">
        {tabs.map((tab) => (
          <button
            ...
            className={`py-2 px-4 rounded font-medium whitespace-nowrap text-sm ${activeTab === tab.key ? "bg-orange-500 text-white" : "text-gray-700 hover:bg-gray-100"}`}
          >
            {tab.label}
          </button>
        ))}
        ...
        {courses[activeTab].map((course, index) => {
            ...
            return (
              <div key={index} className="rounded-2xl p-6 flex flex-col items-center justify-between space-y-4 shadow-[inset_0_0_40px_10px_rgba(0,0,0,0.15)] bg-white min-w-[300px] sm:min-w-[400px] md:min-w-[500px] lg:min-w-[600px] h-[350px] sm:h-[380px] mb-6">
                ...
                <div className="flex items-start space-x-2">
                  ...
                  <p className="text-sm sm:text-lg">{course.date}</p>
                </div>
                <div className="flex items-center text-gray-600 text-sm sm:text-lg">
                  <FaRegCalendarAlt />
                  <span className="ml-2">{course.address}</span>
                </div>
          ...
          return (
            <div key={index} className="rounded-2xl p-6 flex flex-col items-center justify-between space-y-4 shadow-[inset_0_0_40px_10px_rgba(0,0,0,0.15)] bg-white min-w-[300px] sm:min-w-[400px] md:min-w-[500px] lg:min-w-[600px] h-[350px] sm:h-[380px] mb-6">
              ...
              <p className="text-sm sm:text-lg text-center">{course.description} <span className="text-orange-500">{course.extra}</span></p>
              <div className="flex items-center text-gray-600 text-sm sm:text-lg">
                <FaRegCalendarAlt />
                <span className="ml-2">{translatedSchedule(course.schedule)}</span>
              </div>
